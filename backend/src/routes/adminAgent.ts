...
import { authenticate, requireAdminOrAgent, tryRefreshToken } from "../middleware/auth";
...
adminAgentRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const validationResult = ProductInputSchema.safeParse(productDataRaw);
      ...
      try {
        ...
        if (prismaError.message?.includes("not found in enum") || prismaError.code === "P2003") {
          ...
          const rawOrders = await prisma.$queryRaw<any[]>`
            ...
            WHERE ${Prisma.raw(whereClause)}
            GROUP BY o.id
            ORDER BY o.${Prisma.raw(sortFieldSql)} ${Prisma.raw(sortDirSql)}
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (const order of orders) {
        for (const item of order.items) {
          ...
          try {
            ...
            if (product) {
              const itemCost = (product as any).cost ? Number((product as any).cost) : 0;
      ...
      const pendingOrders = orders.filter(o => 
        ["PENDING", "PAID", "PREPARING_TO_SHIP", "READY_TO_SHIP"].includes(o.status)
      ).length;
...
adminAgentRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let inventoryItems = await prisma.inventory.findMany({
        where,
        include: {
          product: {
            select: {
      ...
      const productCostMap = new Map(
        productsWithCost.map(p => [p.id, (p as any).cost || null])
      );
      ...
      const categoryStats: Record<string, { totalStock: number; totalReserved: number; totalAvailable: number; itemCount: number }> = {};
      ...
      const allProductsWithCost = await prisma.product.findMany({
        where: { id: { in: allProductIds } }
      });

      const allProductCostMap = new Map(
        allProductsWithCost.map(p => [p.id, (p as any).cost || 0])
      );
...
adminAgentRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let inventoryToUpdate: any = null;
      let inventoryType = "";
      ...
      if (variantId) {
        ...
        if (!inventoryToUpdate) {
          ...
          inventoryToUpdate = await prisma.inventory.create({
            data: {
              variantId: variantId,
      ...
      const adjustment = adjustInventory(inventoryToUpdate, quantityDelta);
